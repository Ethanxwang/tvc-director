...
| 人物在分镜中的出镜方式 | 资产图格式 | 说明 |
|---------------------|----------|------|
| 全身多次出镜（含面部） | **完整三视图**：面部特写 + 全身正面 + 全身背面 | 标准格式 |
| 全身多次出镜（无面部/遮挡面部） | **造型展示图**：全身正面 + 全身侧面 + 全身背面 + 关键配饰特写 | 身份型品牌世界常用——面部可遮挡/下半脸/侧颜，重点锁定服装和配饰 |
| 只有上半身 | 半身正面 + 面部特写 | 不做全身 |
| 下半身/局部多次出镜（腿、脚、手臂） | **局部造型图**：下半身正面 + 下半身侧面 + 鞋/配饰特写 | 跑鞋/裤装类 TVC 常用 |
| 只有手部与产品交互 | 手部特写参考（肤色、指甲、配饰） | 首饰/手表/护肤品常用 |
| 仅 1-2 格远景剪影/背影 | **不做资产图**，用出镜者标准描述代替 | 见 Part 3 角色一致性 |

#### 角色资产图提示词示例

**示例 1：纯文字生成（无参考图）——古风汉服模特**
...
**示例 2：有脸部参考图 + 有产品/服装参考图——时尚穿搭多格展示**
